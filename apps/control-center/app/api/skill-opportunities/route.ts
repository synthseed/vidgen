...
export async function GET(request: Request) {
  ...
  const rate = checkRateLimit(request, 'skill-opportunities');
  ...
  return NextResponse.json({ generatedAt: new Date().toISOString(), range, opportunities: data.skillOpportunities });
}
